...
import 'package:taxi_booking/screens/WebViewScreen.dart';
...
class PaymentScreen extends StatefulWidget {
  ...
  PaymentScreen({this.amount, this.flow});
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  String? selectedPaymentType, stripPaymentKey, stripPaymentPublishKey, payStackPublicKey, payPalTokenizationKey, flutterWavePublicKey, flutterWaveSecretKey, flutterWaveEncryptionKey, payTabsProfileId, payTabsServerKey, payTabsClientKey, myFatoorahToken, paytmMerchantId, paytmMerchantKey;
  ...
  void payPalPayments() async {
    ...
    if (accessToken == null || accessToken.isEmpty) {
      appStore.setLoading(false);
      toast("PayPal authentication failed. Please check payment configuration.", length: Toast.LENGTH_LONG);
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        if (errorBody['details'] != null && errorBody['details'].isNotEmpty) {
          errorMsg = "Payment failed: ${errorBody['details'][0]['description'] ?? errorBody['message'] ?? 'Unknown error'}";
        } else if (errorBody['message'] != null) {
          errorMsg = "Payment failed: ${errorBody['message']}";
        }
        toast(errorMsg, length: Toast.LENGTH_LONG);
        print("PayPal error: ${response.statusCode} - ${response.body}");
        appStore.setLoading(false);
        return;
      }
    } catch (e) {
      toast("Payment failed: ${e.toString()}", length: Toast.LENGTH_LONG);
      appStore.setLoading(false);
      return;
    }
  }
