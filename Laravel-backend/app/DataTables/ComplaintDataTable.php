    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('driver_id', function ($complaint) {
                return $complaint->driver_id != null
                    ? '<a href="' . route('driver.show', $complaint->driver_id) . '">' . optional($complaint->driver)->display_name . '</a>'
                    : '';
            })
            ->editColumn('ride_request_id', function ($query) {
                return '<a href="'.route('riderequest.show',$query->id).'">'.$query->ride_request_id.'</span></a>';
            })
            ...
            ->editColumn('rider_id' , function ( $complaint ) {
                return $complaint->rider_id != null 
                ? '<a href="' . route('rider.show', $complaint->rider_id) . '">' . optional($complaint->rider)->display_name : '';
            })

            ->editColumn('corporate_id', function($row){
                return $row->corporate_id != null ? '<a href ="' . route('corporate.show', $row->corporate_id) . '">' . optional($row->corporate)->Fullname . '</a>' : '' ;
            })
            ...
            ->rawColumns([ 'action', 'status','ride_request_id','driver_id','rider_id' ,'corporate_id']);
    }
