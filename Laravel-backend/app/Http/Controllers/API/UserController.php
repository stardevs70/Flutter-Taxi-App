...
{
    Log::info('User registration initiated', ['request_data' => $request->all()]);
    ...
    try {
        ...
        Log::debug('Role assigned to user', ['role' => $input['user_type']]);

        if ($request->has('user_detail') && $request->user_detail != null) {
            $user->userDetail()->create($request->user_detail);
            Log::info('User detail created', ['user_id' => $user->id]);
        }
        ...
        Log::info('API token created and profile image fetched', ['user_id' => $user->id]);
        ...
        return json_custom_response([
            'message' => __('message.something_wrong'),
            'success' => false
        ], 500);
    }
}
