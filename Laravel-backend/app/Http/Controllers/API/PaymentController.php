...
class PaymentController extends Controller
{
    ...
    public function paymentSave(Request $request)
    {
        ...
        if ($bid_amount > 0) {
            $ride_request_amount = $bid_amount;
        ...
        $data['fleet_commission'] = $fleet_commission;
        $driver_fee = $ride_request_amount - $admin_commission - $fleet_commission;
        $data['driver_fee'] = $driver_fee;
        $data['driver_tips'] = $driver_tips;
        $data['driver_commission'] = $driver_fee + $ride_request->extra_charges_amount;
    ...
    public function DriverEarningList(Request $request)
    {
        ...
        if( $type == 'today' ) {
            ...
            $total_online_ride = Payment::myPayment()->where('payment_status','paid')->where('payment_type', 'online')->whereDate('datetime',$today)->sum('driver_commission');
            $response = [
                'total_cash_ride'   => $total_cash_ride,
                'total_wallet_ride' => $total_wallet_ride,
                'total_online_ride'   => $total_online_ride,
                'today_earnings'    => $total_cash_ride + $total_wallet_ride + $total_online_ride,
        ...
        if( $type == 'week' ) {
            ...
            $total_online_ride = Payment::myPayment()->where('payment_status', 'paid')->where('payment_type', 'online')->whereBetween('datetime',[ $from_date, $to_date ])->sum('driver_commission');
            ...
            $response = [
                ...
                'total_earnings'    => $total_cash_ride + $total_wallet_ride + $total_card_ride + $total_online_ride,
            ];
        }
        ...
        if( $type == 'report' )
        {
            ...
            $response = [
                ...
                'total_wallet_ride' => $total_wallet_ride,
                'total_online_ride' => $total_online_ride,
                'total_earnings'    => $total_cash_ride + $total_wallet_ride + $total_online_ride,
            ];
        }
        return json_custom_response($response);
    }
