<x-master-layout :assets="$assets ?? []">
                        ...
                        <div class="form-group col-md-4">
                            <div class="row">
                                ...
                                <div class="form-group col-md-12"  id="schedule_wrapper">
                                    ...
                                    {{ html()->text('schedule_datetime', now() ?? old('schedule_datetime'))->class('form-control datetimepicker')->placeholder(__('message.pickup_date_time')) }}
                                </div>
                                <div class="form-group col-md-12">
                                ...
                                <div class="form-group col-md-12">
                                    {{ html()->label(__('message.drop_address') . ' <span class="text-danger">*</span>','end_address')->class('form-control-label') }}
                        ...
                        <div class="form-group col-md-4">
                            <div class="row">
                                ...
                                <div class="form-group col-md-12 corporate_div">
                                    ...
                                    {{ html()->select('corporate_id', isset($data) ? [optional($data->corporate)->id => optional($data->corporate)->full_name] : [])
                                        ->class('form-control select2js')
                                        ->attribute('data-ajax--url', route('ajax-list', ['type' => 'corporate']))
                                        ->attribute('data-placeholder', __('message.select_field', ['name' => __('message.corporate')]))
                                    }}
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-12">
                                    ...
                                    {{ html()->select('service_id', isset($data) ? [ optional($data->service)->id => optional($data->service)->display_name ] : [], old('service_id'))
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    {{ html()->select('driver_id', isset($data) ? [ optional($data->driver)->id => optional($data->driver)->display_name ] : [], old('driver_id'))
                                        ->class('select2js form-group driver')
                                        ->attribute('data-placeholder', __('message.select_name', ['select' => __('message.driver')])) 
                                    }}
                                </div>
                            </div>
                            <div class="row book_a_ride_detail">
                                <div class="form-group col-md-12">
                                    {{ html()->label(__('message.rider'),'rider_id')->class('form-control-label') }}
                                    {{ html()->select('rider_id', isset($data) ? [optional($data->rider)->id => optional($data->rider)->display_name] : [])
                                        ->class('form-control select2js')
                        ...
                        <div class="form-group col-md-4 book_a_ride_detail">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    {{ html()->number('passenger',old('passenger'))->class('form-control')->attribute('min',0)->placeholder(__('message.passenger')) }}
                                </div>
                                <div class="form-group col-md-6">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    {{ html()->select('payment_method', ['cash' => __('message.cash'),'wallet' => __('message.wallet'),'cash_wallet' => __('message.cash_wallet')], old('payment_method'))->class('form-control select2js')->required() }}
                                </div>
                                <div class="form-group col-md-12">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <a href="#" class="supplier_payout_btn loadRemoteModel">
    ...
    @section('bottom_script')
        <script>
            ...
            let locations = { start: null, end: null };
        
            async function initMap() {
                const [{ Map }, { Marker }, { PlaceAutocompleteElement }] = await Promise.all([
                ...
                map = new Map(document.getElementById("map"), {
                    zoom: 14,
                    center: { lat: 22.310696, lng: 70.802288 },
                ...
                initAutocomplete('start_address_wrapper', 'start_address', onStartSelected);
                initAutocomplete('end_address_wrapper', 'end_address', onEndSelected);
            }

            function initAutocomplete(wrapperId, inputId, selectHandler) {
                const wrapper = document.getElementById(wrapperId);
                if (wrapper && !wrapper.querySelector('gmpx-placeautocomplete')) {
                    const autocomplete = new google.maps.places.PlaceAutocompleteElement();
                    autocomplete.id = inputId;
                    autocomplete.setAttribute('name', inputId);
                    autocomplete.locationBias = map.getCenter();
                    wrapper.innerHTML = '';
            ...
            async function onStartSelected({ placePrediction }) {
                ...
                locations.start = { lat: loc.lat(), lng: loc.lng() };
            
                if (startMarker) startMarker.setMap(null);
                startMarker = new google.maps.Marker({ map, position: loc, label: "1" });
            
                map.setCenter(loc);
                serviceList(loc.lat(), loc.lng());
                if (locations.end) { drawRoute(); getFareEstimate(); }
            }
            
            async function onEndSelected({ placePrediction }) {
                const place = placePrediction.toPlace();
                ...
                locations.end = { lat: loc.lat(), lng: loc.lng() };
            
                if (endMarker) endMarker.setMap(null);
                endMarker = new google.maps.Marker({ map, position: loc, label: "2" });
                ...
                if (locations.start) { drawRoute(); getFareEstimate(); }
            }
            ...
            function drawRoute() {
                ...
                $.post("{{ url('api/snap-to-roads') }}", { path }, function (data) {
                    ...
                    if (encoded) {
                        ...
                        } catch (e) { fallbackToStraightLine(); }
                    } else fallbackToStraightLine();
            ...
            function getFareEstimate() {
                ...
                const data = {
                    ...
                    drop_latlng: locations.drop ? [{ lat: locations.drop.lat, lng: locations.drop.lng }] : [],
                    id: serviceId
                };
                ...
                $.post("{{ url('api/estimate-price-time') }}", data, function (response) {
                    if (response?.data?.length) {
                        ...
                        updateFareDetails(service);
                        if ($('#type').val() === 'transport') scheduleDatetime();
                    } else {
            ...
            function updateFareDetails(service) {
                ...
                const formattedDuration = `${Math.floor(mins / 60)} hr ${Math.round(mins % 60)} min`;
                $("#duration").text(formattedDuration);
                $(".duration").val(formattedDuration);
            
                const fleetFare = service.total_amount.toFixed(2);
                $("#fleet_fare").text(fleetFare);
                $(".total_amount").val(fleetFare);
                adjustPriceDataClear();
            
                $('.supplier_payout_btn').attr('href', `{{ route('supplier.payout') }}?fleet_fare=${fleetFare}`);
            }
        
            function scheduleDatetime() {
                if ($('#data_model_estimate_type').val() === 'update') return;
            
                const data = {
                    date_time: $('#schedule_datetime').val(),
                    service_id: $('#service_id').val(),
                    service_type: $('#type').val(),
                    weight: $('#weight').val(),
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status) {
                            ...
                            $('.supplier_payout_btn').attr('href', `{{ route('supplier.payout') }}?fleet_fare=${response.total_amount.toFixed(2)}`);
                        } else {
            ...
            function serviceList(lat, lng) {
                const type = $('#type').val();
                const route = `{{ route('ajax-list', ['type' => 'service_for_ride']) }}&latitude=${lat}&longitude=${lng}&dispatch_service=${type}`;
            ...
            function driverList(serviceId) {
                ...
                const route = `{{ route('ajax-list', ['type' => 'driver_for_ride']) }}&service_id=${serviceId}&latitude=${latitude}&longitude=${longitude}`;
            
                $.get(route.replaceAll('amp;', ''), function (result) {
                    $('.driver').select2({
                        width: '100%',
            ...
            function resetServiceFields() {
                $('#distance, #duration, #fleet_fare').text('');
                $('.distance, .duration, .total_amount').val('');
                $('#distance').val('');
            }
            
            function adjustPriceDataClear() {
                $('#fleet_fare_change, #estimate_fleet_fare, #modal_base_fare, #modal_surcharge, #modal_discount, #reason').val('');
                $('#fleet_fare_change').text('');
            }
            
            function bookRide() {
                if ($('#type').val() === 'transport') {
                    $('.book_a_ride_div, .transport_detail').fadeIn(500);
                    $('.book_a_ride_detail').fadeOut(100);
                } else {
                    $('.book_a_ride_div, .transport_detail').hide();
                    $('.book_a_ride_detail').show();
                }
            }
            
            function travelerInfo() {
                $('.corporate_div').toggle($('input[name="traveler_info"]:checked').val() === 'corporate');
            }
        
            function toggleFieldsByType() {
                const type = $('#type').val();
                $('#weight_wrapper').toggle(type === 'transport');
            }
            ...
            $(document).ready(function () {
                ...
                $('#type').on('change', function () {
                    ...
                    if (lat && lng) serviceList(lat, lng);
                    bookRide();
                    if (this.value === 'transport') scheduleDatetime();
                });
                ...
                $('#service_id').on('change', function () {
                    ...
                    if (this.value) driverList(this.value);
                });
                ...
                $('.driverList').on('click', function () {
                    const serviceId = $('#service_id').val();
                    if (serviceId) driverList(serviceId);
                    else $('.driver').empty();
                });
            
                $('input[name="traveler_info"]').on('change', travelerInfo);
