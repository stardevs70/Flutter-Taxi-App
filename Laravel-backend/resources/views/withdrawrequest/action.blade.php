...
@endif
    ...
    {{-- {!! html()->form('DELETE', route('withdrawrequest.destroy', $query->id))->attribute('data--submit', 'withdrawrequest'.$query->id)->open() !!}

        <a class="mr-2 text-danger" href="javascript:void(0)"
            data--submit="withdrawrequest{{ $query->id }}"
            data--submit="confirm_form"
        data--confirmation="true"
        data-title="{{ __('message.delete_form_title', ['form' => __('message.withdrawrequest') ]) }}"
