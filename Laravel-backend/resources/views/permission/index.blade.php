                            ...
                            <div class="card mb-2 border-radius-20">
                                <div class="card-header border-radius-20 d-flex justify-content-between collapsed btn" id="heading_{{$key}}" data-toggle="collapse" data-target="#pr_{{$key}}" aria-expanded="false" aria-controls="pr_{{$key}}">
                                    <div class="header-title">
                                        <h6 class="mb-0 text-capitalize"> <i class="fa fa-angle-double-down text-dark mr-10"></i> {{ ucwords(str_replace(['_', '-'],' ', $data->name )) }}<span class="badge badge-secondary"></span></h6>
                                    </div>
                                </div>
                                        ...
                                        <table class="table text-center table-bordered bg_white">
                                            <tr>
                                                ...
                                                @foreach($data->subpermission as $p)
                                                    <th class="text-capitalize">{{ ucwords(str_replace(['_', '-'],' ', $p->name)) }}</th>
                                                @endforeach
                                            </tr>
