...
class DashboardScreenState extends State<DashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> getCurrentRequest() async {
    getCurrentRideRequest().then((value) async {
      ...
      if (value.onRideRequest != null) {
        ...
        // No current ride request from server - clear any stale local data
        passengerCount = value.rideRequest?.passenger ?? 0;
        luggageCount = value.rideRequest?.luggage ?? 0;

        // If we had local data but server says no current ride, clear it
  ...
  getNewRideReq(int? riderID, {bool? refresh}) async {
    ...
    await rideDetail(rideId: riderID).then((value) async {
      ...
      appStore.setLoading(false);
      String errorMsg = error.toString().toLowerCase();
      // If ride not found, clear stale data
      if (errorMsg.contains('not found') ||
          errorMsg.contains('no longer') ||
          errorMsg.contains('expired') ||
  ...
  Future<void> rideRequest({String? status}) async {
    ...
    await rideRequestUpdate(request: req, rideId: servicesListData!.id).then((value) async {
      ...
      // Update Firebase to notify the user app about status change
  ...
  Future<void> rideRequestAccept({bool deCline = false}) async {
    ...
    await rideRequestResPond(request: req).then((value) async {
      ...
      if (deCline) {
        ...
        sharedPref.remove(IS_TIME2);
        setMapPins();
      } else if (servicesListData?.isSchedule == 1) {
        // Scheduled ride accepted - show success and clear UI
        toast('Scheduled ride accepted! Check your upcoming rides.');
        rideService.updateStatusOfRide(rideID: servicesListData!.id, req: {
          'on_stream_api_call': 0,
        });
        servicesListData = null;
      ...
      appStore.setLoading(false);
      String errorMsg = error.toString().toLowerCase();
      // Check if the error indicates the ride no longer exists (stale data)
      if (errorMsg.contains('not found') ||
          errorMsg.contains('no longer') ||
          errorMsg.contains('expired') ||
          errorMsg.contains('already') ||
          errorMsg.contains('invalid')) {
        // Clear stale data and refresh from server
        toast('Ride was already taken by another driver or cancelled.');
        servicesListData = null;
        _polyLines.clear();
        sharedPref.remove(ON_RIDE_MODEL);
        sharedPref.remove(IS_TIME2);
            ...
            StreamBuilder<QuerySnapshot>(
                ...
                builder: (c, snapshot) {
                  if (snapshot.hasError) {
                    // Log error but don't show on UI - Firestore permission errors are handled gracefully
                    print("Firestore stream error: ${snapshot.error}");
                    return SizedBox(); // Return empty widget instead of showing error
                  }
                  if (snapshot.hasData) {
                                              ...
                                              children: [
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    inkWellWidget(
                                                      onTap: () {
                                                        if (servicesListData?.startLatitude != null &&
                                                            servicesListData?.startLongitude != null &&
                                                            servicesListData?.endLatitude != null &&
                                                            servicesListData?.endLongitude != null) {
                                                          openMap(
                                                            double.parse(servicesListData!.startLatitude.toString()),
                                                            double.parse(servicesListData!.startLongitude.toString()),
                                                            double.parse(servicesListData!.endLatitude.toString()),
                                                            double.parse(servicesListData!.endLongitude.toString()),
                                                          );
                                                        }
                                                      },
                                                ...
                                                // Extra charges option removed per client request
                                                buttonWidget()
                                              ],
                                            ),
                                          ),
            ...
            child: AppButtonWidget(
              ...
              onTap: () async {
                if (await checkPermission()) {
                  // Only show payment collection dialog for CASH payments that haven't been paid
  ...
  // Display extra booking options
  Widget _buildExtrasView() {
    if (servicesListData == null) return SizedBox();

    List<Widget> extras = [];

    if (servicesListData!.tripProtection == 1) {
      extras.add(_buildExtraItem('Trip Protection', servicesListData!.tripProtectionPrice));
    }
    if (servicesListData!.meetAndGreet == 1) {
      String meetGreetInfo = 'Meet & Greet';
      if (servicesListData!.meetGreetName != null && servicesListData!.meetGreetName!.isNotEmpty) {
        meetGreetInfo += ': ${servicesListData!.meetGreetName}';
      }
      extras.add(_buildExtraItem(meetGreetInfo, servicesListData!.meetAndGreetPrice));
    }
    if (servicesListData!.travelingWithPet == 1) {
      extras.add(_buildExtraItem('Traveling with Pet', servicesListData!.travelingWithPetPrice));
    }
    if (servicesListData!.childSeat == 1) {
      String seatInfo = 'Child Seat';
      List<String> seatDetails = [];
      if ((servicesListData!.boosterSeatCount ?? 0) > 0) seatDetails.add('${servicesListData!.boosterSeatCount} Booster');
      if ((servicesListData!.rearFacingInfantSeatCount ?? 0) > 0) seatDetails.add('${servicesListData!.rearFacingInfantSeatCount} Infant');
      if ((servicesListData!.forwardFacingToddlerSeatCount ?? 0) > 0) seatDetails.add('${servicesListData!.forwardFacingToddlerSeatCount} Toddler');
      if (seatDetails.isNotEmpty) seatInfo += ' (${seatDetails.join(', ')})';
      extras.add(_buildExtraItem(seatInfo, servicesListData!.childSeatPrice));
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 4),
          ...extras,
          if (servicesListData!.extrasAmount != null && servicesListData!.extrasAmount! > 0)
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text('\$${servicesListData!.extrasAmount!.toStringAsFixed(2)}', style: boldTextStyle(size: 12, color: Colors.green)),
                ],
              ),
            ),
        ],
