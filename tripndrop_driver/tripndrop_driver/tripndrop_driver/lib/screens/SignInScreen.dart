      ...
      if (isAcceptedTc) {
        ...
        await logInApi(req).then((value) async {
          ...
          try {
            ...
            sharedPref.setString(UID, userCredential.user!.uid);
            updateProfileUid();
            ...
            if (e.toString().contains('user-not-found')) {
              // User doesn't exist in Firebase - create Firebase account
              ...
              // Firebase user exists but password is different - proceed without Firebase
              // Laravel auth already passed, so allow login
              log("Firebase password mismatch - proceeding with Laravel auth only");
              appStore.setLoading(false);
              if (sharedPref.getInt(IS_Verified_Driver) == 1) {
                await checkPermission().then((value) async {
                  await Geolocator.getCurrentPosition().then((value) {
              ...
              // Other Firebase error - still allow login since Laravel auth passed
              appStore.setLoading(false);
              if (sharedPref.getInt(IS_Verified_Driver) == 1) {
                await checkPermission().then((value) async {
                  await Geolocator.getCurrentPosition().then((value) {
                   ...
                   Row(
                     children: [
                       SizedBox(
                         height: 18.0,
                         width: 18.0,
                         child: Checkbox(                                   side:  BorderSide(color: Colors.grey.shade400, width: 1.5),
    ...
    return Column(
      children: [
          ...
          child: Row(
            ...
            //  Expanded(child: Divider(color: dividerColor)),
              Padding(
                ...
                child: Text(language.orLogInWith, style: primaryTextStyle().copyWith(color: Colors.grey.shade600)),
              ),Spacer(),
             /* Expanded(
        ...
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            // Google Sign-In removed per client request
            inkWellWidget(
              onTap: () async {
                showDialog(
                  context: context,
