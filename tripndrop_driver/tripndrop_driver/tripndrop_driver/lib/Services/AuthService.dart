  ...
  Future<void> signUpWithEmailPassword(
    ...
    try {
      createAuthUser(email, password, isOtpLogin).then((user) async {
        ...
        appStore.setLoading(false);
        log("createAuthUser error: ${e.toString()}");
        toast(e.toString());
      });
    } on FirebaseException catch (error) {
      appStore.setLoading(false);
...
class GoogleAuthServices {
  final GoogleSignIn googleSignIn = GoogleSignIn(
    scopes: ['email'],
    // Web Client ID from Firebase Console (client_type: 3) - Eagle Rides project
...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> {
  ...
  Future<void> _onVerified(User user) async {
    ...
    if(widget.from!="login") {
      ...
      userModel.displayName = widget. fName.validate() + " " +  widget.lName.validate();
      ...
      if (sharedPref.getInt(IS_Verified_Driver) == 1) {
        ...
        launchScreen(context, DocumentsScreen(), isNewTask: true, pageRouteAnimation: PageRouteAnimation.Slide);
