...
buttonText({String? status, String? paymentType, String? paymentStatus}) {
  if (status == NEW_RIDE_REQUESTED) {
    ...
    return language.arrived;  // First step: Mark as Arrived
    ...
    return language.startRide;  // Second step: Start the ride
  } else if (status == COMPLETED && paymentStatus != PAYMENT_PAID && paymentType == CASH) {
    // Only show "Cash Collected" for cash payments
    return language.cashCollected;
  } else if (status == COMPLETED) {
    // For online payments or already paid, ride is complete
    ...
    if (value.rideSetting != null) {
      value.rideSetting!.forEach((element) {
        ...
        if (element.key == MAX_TIME_FOR_DRIVER_SECOND) {
          REQUEST_TIME_VAL = int.parse(element.value ?? '120');
        }
...
print("aaaaa");
  ...
  OneSignal.Notifications.addClickListener((notification) async {
    ...
    if (notType != null && notId != null && !notId.toString().contains('CHAT')) {
      ...
      // Handle new ride request notification - go directly to dashboard to see the ride
      if (notType == "new_ride_request") {
        print("New ride request notification received, going to Dashboard");
        launchScreen(getContext, DashboardScreen(), isNewTask: true);
        return;
      }

      try {
        // Extract ride_id from either the data field or from notId (RIDE_xxx format)
        int? rideId;
        if (data["ride_id"] != null) {
          rideId = int.tryParse(data["ride_id"].toString());
        } else if (notId.toString().startsWith('RIDE_')) {
          rideId = int.tryParse(notId.toString().replaceAll('RIDE_', ''));
        } else {
          rideId = int.tryParse(notId.toString());
        }

        var value = await rideDetail(rideId: rideId);
        RideDetailModel mRideModel = value;
print("rideeeeid");
        if (mRideModel.data?.driverId != null) {
          print("driverId11");
